...
if __name__ == "__main__":
    ...
    parser.add_argument("--command", help="Command to run (e.g. transfer)")
    parser.add_argument("--to", help="Recipient address")
    parser.add_argument("--amount", help="Amount to send")

    args = parser.parse_args()

    async def cli_transfer():
        if not ld():
            return "wallet.json error"

        if not args.to or not args.amount:
            return "Missing --to or --amount"
        ...
        try:
            ...
            if amount > balance:
                return f"Insufficient balance: {balance:.6f} OCT"

            tx, _ = mk(args.to, amount, nonce + 1)
            ok, tx_hash, dt, _ = await snd(tx)

            return tx_hash if ok else f"ERROR: {tx_hash}"
        except Exception as e:
            return f"Error: {str(e)}"

    async def run_app():
        if args.command == "transfer":
            result = await cli_transfer()
            print(result)
            sys.stdout.flush()
        else:
            await main()
